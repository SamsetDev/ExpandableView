 ...
 */
public class ExpandbleLayout extends LinearLayout {
    ...
    public ExpandbleLayout(Context context) {
        this(context, null);
    }
     ...
     * 'handle' and 'content'
     */
    public ExpandbleLayout(Context context, AttributeSet attrs) {
        super(context, attrs);
        mListener = new DefaultOnExpandListener();

        TypedArray a = context.obtainStyledAttributes(
                attrs, R.styleable.ExpandableLayout, 0, 0);

        // How high the content should be in "collapsed" state
        mCollapsedHeight = (int) a.getDimension(
                R.styleable.ExpandableLayout_collapsedHeight, 0.0f);

        // How long the animation should take
        mAnimationDuration = a.getInteger(
                R.styleable.ExpandableLayout_animationDuration, 500);

        int handleId = a.getResourceId(
                R.styleable.ExpandableLayout_handle, 0);
        ...
        int contentId = a.getResourceId(
                R.styleable.ExpandableLayout_content, 0);
        if (contentId == 0) {
            throw new IllegalArgumentException(
